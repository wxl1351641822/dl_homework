...
def stack_lstm_net(data, input_dim, class_dim=opt.class_num, emb_dim=opt.embedding_size, hid_dim=opt.hidden_units, stacked_num=opt.stacked_num):
    # 计算词向量
    ...
    for i in range(2, stacked_num + 1):
        ...
        lstm, cell = fluid.layers.dynamic_lstm(
            ...
            param_attr=fluid.ParamAttr(
                ...
                initializer=fluid.initializer.TruncatedNormal(scale=0.02, seed=opt.seed)),
            bias_attr=fluid.ParamAttr(
                initializer=fluid.initializer.Constant(0.)),
        )
        inputs = [fc, lstm]
...
def train(lstm_net, i,opt_config):
    s=''
    with log_writter.mode("train%d" % i) as train_log1:
    ...
    with log_writter.mode("test%d" % i) as test_log1:
    ...
    for pass_id in range(num_epochs):
        ...
        if test_acc>best:
            ...
            if(early_stopping==0):
                break
    s=' | '.join([str(train_mean_cost), str(train_mean_acc),str(test_cost), str(test_acc)])
    opt_config.write(' | '+s)
